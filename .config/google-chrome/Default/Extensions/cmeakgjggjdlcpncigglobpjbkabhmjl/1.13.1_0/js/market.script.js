...
const COOKIE_ENABLED_SIH = 'enableSIH_Market';
...
$J('#switchPanel #switcher').change((e) => {
  ...
  SetCookie(COOKIE_ENABLED_SIH, currentTarget.checked, 365);
...
var RemoveListing = function ($parent) {
    var cks = $parent.find('.market_listing_row .select-checkbox:visible:checked');
    if (cks.length > 0) {
        ...
        if (idl.indexOf('mylisting') == 0) {
            ...
            new Ajax.Request('//steamcommunity.com/market/removelisting/' + listingid, {
                method: 'post',
                parameters: {
                    sessionid: g_sessionID
                },
            ...
            new Ajax.Request('//steamcommunity.com/market/cancelbuyorder/', {
                method: 'post',
                parameters: {
                    sessionid: g_sessionID,
...
const parseListings = function () {
    ...
    $ACTIVE_LISTING_ROWS.find('.market_listing_price').children('span').each(function (rowId, price) {
        ...
        var pays = GetPriceValueAsInt(buyerPrice);
        if (!isNaN(pays)) totalSelling += pays;

        var yourPrice = isBuyOrder ? '0.00' : $price.find('span:last').text();
        var recieve = GetPriceValueAsInt(yourPrice.trim().replace('(', '').replace(')', ''));
...
var AddSortableColumns = function (jheader) {
  ...
  jheader.find('> span').each(function () {
      if ($J(this).hasClass('market_listing_edit_buttons') || $J(this).hasClass('market_listing_listed_date')) return;
      ...
      if ($J(this).hasClass('market_listing_buyorder_qty')) {
        ...
        $J(this).click(function () {
          ...
          $rowsli.sort(function (a, b) {
            var an = parseFloat(getNumber($J(a).find('.market_listing_my_price.market_listing_buyorder_qty').text())),
                bn = parseFloat(getNumber($J(b).find('.market_listing_my_price.market_listing_buyorder_qty').text()));
            if (an == bn) {
        ...
        $J(this).click(function () {
          ...
          $rowsli.sort(function (a, b) {
            ...
            const extractPrice = function (row) {
              ...
              return GetPriceValueAsInt(aprice);
            };
...
var FetchAllItemsPrices = function () {
    ...
    getLowestPriceHandler(element.rgItem, '', function (item) {
        ...
        var recieve = GetPriceValueAsInt(p.text());
        var lowest = GetPriceValueAsInt(item.lowestPrice);
        if (lowest < recieve)
            row.addClass('red-bg');
        FetchAllItemsPrices();
    });
...
var addHideListingButton = function () {
  const HIDE_LISTINGS = GetCookie('hideListings') || 'false';
  var btnPanel = `
    <a href="javascript:void(0)" id="btnHideListing" data-hidden="${HIDE_LISTINGS}" class="item_market_action_button item_market_action_button_green" style="float: right; right: 15px; position: relative;">
  ...
  if ($J('#tabContentsMyListings #tabContentsMyActiveMarketListingsTable:not(:has(.market_listing_table_message))').length) {
    $J('#tabContentsMyListings').prepend(btnPanel);
    if (HIDE_LISTINGS === 'true') $J('#tabContentsMyListings #tabContentsMyActiveMarketListingsTable').children('div').hide();
  }
  $J('#btnHideListing').click(function () {
    const isHidden = $J(this).data('hidden');
    const $elem = $J(this);
    if (isHidden) {
      // $J('#tabContentsMyListings').find('.market_listing_row').show();
      // $J('#tabMyListings').removeClass('market_tab_well_tab_inactive');
      // $J('#tabMyListings').addClass('market_tab_well_tab_active');
      $elem.find('.item_market_action_button_contents').text(SIHLang.market.hidelistings);
      $elem.next().children('div').show();
    } else {
      // $J('#tabContentsMyListings').find('.market_listing_row').hide();
      // $J('#tabMyListings').removeClass('market_tab_well_tab_active');
      // $J('#tabMyListings').addClass('market_tab_well_tab_inactive');
      $elem.find('.item_market_action_button_contents').text(SIHLang.market.showlistings);
      $elem.next().children('div').hide();
    }
    SetCookie('hideListings', !isHidden, 7);
    $J(this).data('hidden', !isHidden);
  });
};
