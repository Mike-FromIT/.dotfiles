...
const COOKIE_ENABLED_SIH = 'enableSIH_Inventory';
...
const addQuickSellButton = (item, elActions) => {
    ...
    var inputValue = getNumber(description.lowestPrice);
        ...
        for (var i = 0; i < sItem.description.descriptions.length; i++) {
            ...
            if (d.type == 'html' && d.value.startsWith('<br><div id="sticker_info" name="sticker_info" title="Sticker Details"') && !d.stickers) {
                ...
                for (var i2 = 0; i2 < stickers.length; i2++) {
                    ...
                    var stickerLink = window.location.protocol + '//steamcommunity.com/market/priceoverview/?appid=730&country=' + cc + '&currency=' + currencyId + '&market_hash_name=Sticker | ' + stickers[i2].trim();
...
var ShowQueue = function (goo) {
    ...
    div.find('#bt_lower, #bt_higher').click(function (e) {
        ...
        $J('.queue-item-container:visible').each(function (idx, elem) {
            ...
            if (item && (item.description.lowestPriceW || item.extprice)) {
                var price = item.description.lowestPriceW ? getNumber(item.description.lowestPriceW) : (item.extprice || 0);
    ...
    div.find('#bt_sort_price').click(function (e) {
        ...
        $rowsli.sort(function (a, b) {
            ...
            var an = (typeof (rgIa.description.lowestPriceW) == 'undefined' ? (rgIa.extprice || 0) : getNumber(rgIa.description.lowestPriceW)),
                bn = (typeof (rgIb.description.lowestPriceW) == 'undefined' ? (rgIb.extprice || 0) : getNumber(rgIb.description.lowestPriceW));
    ...
    for (var i = 0; i < it.length; i++) {
        ...
        getLowestPriceHandler(rgItem, function (item) {
            ...
            if (item && item.description.lowestPriceW) {
                var inputValue = getNumber(item.description.lowestPriceW);
                var nAmount = inputValue;
                var priceWithoutFee = null;
                if (inputValue > 0 && nAmount == parseInt(nAmount)) {
                    // Calculate what the seller gets
...
var recalcPrice = function (item) {
    ...
    buyerPrice = rgItem.description.buyerPrice || getNumber(rgItem.description.lowestPriceW);
    ...
    setTimeout(function () {
        ...
        $J('.games_list_tab').click(() => {
            if (hasExternalPricesAPI(g_ActiveInventory.appid)) {
                updateInventoryProviders();
                $J('#pricesPanel').show();
                $J('#Lnk_SortItems').toggle(g_ActiveInventory.appid != 753);
                g_ActiveInventory.LoadCompleteInventory();
            } else {
                $J('#pricesPanel, #Lnk_SortItems').hide();
            }

            $J('#Bt_GetFloat, #sort-types-content #float').toggle(g_ActiveInventory.appid == 730);
            $J('#steamPrice').toggle([730, 578080].includes(g_ActiveInventory.appid));
        });

        $J('#Bt_GetFloat, #sort-types-content #float').toggle(g_ActiveInventory.appid == 730);
        $J('#steamPrice').toggle([730, 578080].includes(g_ActiveInventory.appid));
        $J('#pricesPanel').toggle(hasExternalPricesAPI(g_ActiveInventory.appid));
        $J('#Lnk_SortItems').toggle(g_ActiveInventory.appid != 753 && hasExternalPricesAPI(g_ActiveInventory.appid));
        ...
        if (window.inventoryprice) {
            ...
            $J('body').on('click', '#invValue', function () {
                ...
                if ([730, 578080].includes(g_ActiveInventory.appid)) {
                    const items = [];
                    ...
                    window.chrome.runtime.sendMessage(SIHID, {
                        ...
                        if (res.success) {
                            const sum = items.reduce((acc, x) => (acc += getNumber(res.prices[x])), 0);
...
var SetupExternalDropdown = function (appid) {
    ...
    if (ExternalPrices[appid]) {
        ...
        else favSource = ['csgofast', 'dapubg', 'opskins', 'backpacktf'];
...
var GetAllInventoryPrice = function (inventar) {
    ...
    window.chrome.runtime.sendMessage(SIHID, {
        ...
        if (res.success) $J('#providerPrice .priceValue').text(v_currencyformat(Math.round(res.amount * 100), ccode));
        ...
        PriceQueue.GetPrice({
            ...
            success: function (res) {
                if (res.success && res.lowest_price) {
                    totalPrice += getNumber(res.lowest_price);
